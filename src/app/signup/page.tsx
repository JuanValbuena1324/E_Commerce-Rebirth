...
export default function SignupPage() {
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      if (response.ok) {
        showToast(data.message || `Welcome to Rebirth, ${firstName}!`);
        ...
        <div className="w-full max-w-[420px]">
          ...
          <div className="grid grid-cols-2 gap-3 mb-3">
            <input type="text" placeholder="First Name" value={firstName} onChange={(e) => setFirstName(e.target.value)}
              className="w-full px-4 py-3 border border-brand-light bg-transparent font-sofia text-[13px] text-brand-black focus:outline-none focus:border-brand-black transition-colors" />
          ...
          <Link href="/login" className="block text-center font-cinzel text-[9px] tracking-[0.15em] uppercase text-brand-mid hover:text-brand-black transition-colors mt-6">
